...
[icc_nps,icc_mean,icc_se, weight,id] = icc_by_study_by_trial(all_data, 'nps');
id_icc = [uniq_study_id;'mean';'se'];
icc = [icc_nps,icc_mean,icc_se];
NPS_ICC = array2table(icc,'VariableNames',id_icc);
cd('../behavior')
writetable(NPS_ICC, 'NPS_ICC_exclude_nsf.csv');

%%
uniq_study_id = unique(all_data.study_id,'rows','stable');
[icc_rating,icc_mean_rating,icc_se_rating, weight_rating,id] = icc_by_study_by_trial(all_data, 'rating');
id_icc = [uniq_study_id;'mean';'se'];
icc_all = [icc_rating,icc_mean_rating,icc_se_rating];
rating_ICC = array2table(icc_all,'VariableNames',id_icc);
cd('../behavior')
writetable(rating_ICC, 'rating_ICC_exclude_nsf.csv');
