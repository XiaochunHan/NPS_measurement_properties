...
File = 'metadata_all_NPS_complete.mat';
cd(Path);
load(File);
%%
uniq_study_id = unique(all_data.study_id,'rows','stable');
[icc_single,icc_contrast,id] = icc_by_study_by_contrast(all_data, 'nps');
uniq_temp_id = unique(id(2:end,:));
temp = uniq_temp_id(uniq_temp_id~=0);
table_icc_single = zeros(length(temp),length(uniq_study_id));
table_icc_contrast = zeros(length(temp),length(uniq_study_id));
for s = 1:length(uniq_study_id)
    for i = 2:size(icc_single,1)
        for t = 1:length(temp)
            if id(i,s) == temp(t,1)
               table_icc_single(t,s) = icc_single(i,s);
...
NPS_ICC_single = array2table([temp,table_icc_single],'VariableNames',['temp';uniq_study_id]);
NPS_ICC_contrast = array2table([temp,table_icc_contrast],'VariableNames',['temp';uniq_study_id]);
cd('../behavior')
writetable(NPS_ICC_single, 'NPS_ICC_single.csv');
writetable(NPS_ICC_contrast, 'NPS_ICC_contrast.csv');

%%
uniq_study_id = unique(all_data.study_id,'rows','stable');
[icc_single,icc_contrast,id] = icc_by_study_by_contrast(all_data, 'rating');
uniq_temp_id = unique(id(2:end,:));
temp = uniq_temp_id(uniq_temp_id~=0);
table_icc_single = zeros(length(temp),length(uniq_study_id));
table_icc_contrast = zeros(length(temp),length(uniq_study_id));
...
rating_ICC_single = array2table([temp,table_icc_single],'VariableNames',['temp';uniq_study_id]);
rating_ICC_contrast = array2table([temp,table_icc_contrast],'VariableNames',['temp';uniq_study_id]);
cd('../behavior')
writetable(rating_ICC_single, 'rating_ICC_single.csv');
writetable(rating_ICC_contrast, 'rating_ICC_contrast.csv');

%%
function [icc_single,icc_contrast,id_shrink] = icc_by_study_by_contrast(all_data, varname)
    ...
    for con = 1:size(mean_by_study_by_temp,1)
        if ~isempty(mean_by_study_by_temp{con,i})
           ...
           if c ~= 1
              ...
              icc_single(c,i) = ICC(3, 'k', extract_match{c,i});
