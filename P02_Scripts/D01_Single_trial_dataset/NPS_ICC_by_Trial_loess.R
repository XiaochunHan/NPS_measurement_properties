...
#
file = '/Users/spring/Dropbox (Dartmouth College)/NPS_Reliability/D01_Single_trial_dataset/behavior/NPS_ICC.csv'
...
lines(x[1:28],plx.bmrk3$fit, col=alpha(col[1],0.3), lwd=3)
...
lines(x[1:39],plx.bmrk4$fit, col=alpha(col[2],0.3), lwd=3)
...
lines(x[1:18],plx.bmrk5$fit, col=alpha(col[3],0.3), lwd=3)
...
lines(x[1:31],plx.exp$fit, col=alpha(col[4],0.3), lwd=3)
...
lines(x[1:24],plx.ie$fit, col=alpha(col[5],0.3), lwd=3)
...
lines(x[1:35],plx.ie2$fit, col=alpha(col[6],0.3), lwd=3)
...
lines(x[1:32],plx.ilcp$fit, col=alpha(col[7],0.3), lwd=3)
...
lines(x[1:23],plx.nsf$fit, col=alpha(col[8],0.3), lwd=3)
...
lines(x[2:48],plx.scebl$fit, col=alpha(col[9],0.3), lwd=3)
...
lines(x[1:40], plx.mean$fit, col="black", lwd=3)
abline(h = 0.75, col = "grey",lty = 2, lwd = 1.5)
#abline(v = 20, col = "grey",lty = 2, lwd = 1.5)
abline(v = 30, col = "grey",lty = 2, lwd = 1.5)
...
# dot of RTNF
points(x=15, y = 0.75, col = "#BF7FBA", pch = 16,cex=4)
text(locator(), labels = "RTNF")
# dot of OLP4CBP
points(x=2.5, y = 0.46, col = "#96AFCC", pch = 16,cex=4)
...
tiff("NPS_ICC_by_trial.tiff",units="in",width = 8,height = 12,res = 300)
dev.off()
