function [stats_table, within_b, within_r] = plugin_regress_between_subject(X, Y, figname)
%
...
%         isok{j}(isnan(x{j}) | isnan(y{j}) | x{j} == 0 | y{j} == 0) = false;
        ...
        [wasnan, x, y] = nanremove(double(reshape(cell2mat(x),[],1)), double(reshape(cell2mat(y),[],1)));
        
        % sometimes x or y is constant for some studies, so r and b are
        % undefined.  
        if all(x == x(1)) || all(y == y(1)) || all(wasnan)
            [rr, pp] = deal(NaN);
            b = [NaN; NaN];
...
%     end
    ...
    if all(isnan(within_b{i})) % no valid data
        ...
        [t_by_study(i, 1), df_by_study(i, 1), p_by_study(i, 1), within_person_r(i, 1), within_person_b(i, 1), within_person_b_ste(i, 1)] = deal(NaN);
