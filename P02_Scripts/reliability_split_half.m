function [firstsecondhalf_r_spearmanbrown, tabledat] = reliability_split_half(celldat, varargin)
...
%     robmeans(j, 2) = robust_mean(celldat{j}(wh2));
    ...
    robmeans12(j, 1) = robust_mean(celldat{j}(wh1));
    robmeans12(j, 2) = robust_mean(celldat{j}(wh2));
...
[firstsecondhalf_robustmean_r, firstsecondhalf_robustmean_p] = corr(robmeans12(:, 1), robmeans12(:, 2));
...
tabledat = table(mean_trials, firstsecondhalf_r_spearmanbrown, firstsecondhalf_p, ...
    firstsecondhalf_r_robustmean_spearmanbrown, firstsecondhalf_robustmean_p);
