...
#
file = '/Users/spring/Dropbox (Dartmouth College)/NPS_Reliability/D01_Single_trial_dataset/behavior/rating_ICC_exclude_nsf.csv'
Data = read.csv(file,header=TRUE)
setwd('/Users/spring/Dropbox (Dartmouth College)/NPS_Reliability/Manuscript/Figures')
tiff("Rating_ICC_by_trial_exclude_nsf.tiff",units="in",width = 10,height = 12,res = 300)
x = 1:48
# bmrk3 ICC
...
lines(x[1:28],plx.bmrk3$fit, col=alpha(col[1],0.6), lwd=3)
...
lines(x[1:39],plx.bmrk4$fit, col=alpha(col[2],0.6), lwd=3)
...
lines(x[1:18],plx.bmrk5$fit, col=alpha(col[3],0.6), lwd=3)
...
lines(x[1:31],plx.exp$fit, col=alpha(col[4],0.6), lwd=3)
...
lines(x[1:24],plx.ie$fit, col=alpha(col[5],0.6), lwd=3)
...
lines(x[1:35],plx.ie2$fit, col=alpha(col[6],0.6), lwd=3)
...
lines(x[1:32],plx.ilcp$fit, col=alpha(col[7],0.6), lwd=3)
...
lines(x[2:48],plx.scebl$fit, col=alpha(col[8],0.6), lwd=3)
...
abline(h = 0.75, col = "grey",lty = 2, lwd = 3)
...
# dot of RTNF
points(x=15, y = 0.86, col = "#BF7FBA", pch = 16,cex=6)
